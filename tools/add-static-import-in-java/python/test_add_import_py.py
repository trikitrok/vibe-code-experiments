#!/usr/bin/env python3
# pytest tests for dev/python/add-import.py (class imports)
#
# Validates:
# 1) Adding a class import when missing
# 2) Idempotency when the class import is already present
#
# Usage (from project root):
#   pytest -q dev/python/test_add_import_py.py
...
IMPORT_LINE = f"import {CLASS_FQN};"


@pytest.mark.parametrize("pkg,cls", [("com.example", "MissingImport")])
def test_adds_class_import_when_missing(tmp_path: Path, pkg: str, cls: str, request: pytest.FixtureRequest, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    # Run the Python script to add the class import (default mode)
    result = subprocess.run(
        [sys.executable, str(SCRIPT), CLASS_FQN, str(java_file)],
        capture_output=True,
        text=True,
        check=False,
    )
    ...
    # Output should report addition exactly as the shell version
...
def test_idempotent_when_class_import_already_exists(tmp_path: Path, cls: str, request: pytest.FixtureRequest, capsys: pytest.CaptureFixture[str]) -> None:
